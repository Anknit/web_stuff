...
*/
	...
	private function Prepare_Output($output, $output_Format)
	{
	...
	private function Prepare_Query($input_array, $prepare_For)
	{
		...
		switch($prepare_For)
		{
			case 'READ':
				...
				if(isset($input_array['MatchTable']) && $input_array['MatchTable'] != "" && $input_array['MatchTable'] !== NULL)
				{
					for($i = 0; $i< count($input_array['MatchTable']); $i++){ // Match table is array of arrays. The inner arrays would refer How to join which table and on which condition 
						$joinThis	=	$input_array['MatchTable'][$i];
						if(isset($joinThis['Table']) && isset($joinThis['clause'])){
							if(isset($joinThis['Collect'])){
								$Query	.=	" ".$joinThis['Collect']." JOIN ";
							}
							else{
								$Query	.=	" INNER JOIN ";
							}
							
							$Query	.=	" ".$joinThis['Table']."  ON ".$joinThis['clause']." ";
