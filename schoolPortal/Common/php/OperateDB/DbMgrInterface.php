...
/*=====================================================PREPARE INPUT FOR READ METHOD==============================================================*/
/*
 * @access public
* @param  $inputType as a integer. 1 refers to main input array , 2 refers to inner array required for match table
* InputType 2 is used to prepare sub input arrays of read input.
* The MatchTable array in DB_Read array refers to arrays consisting of table, join types(Collect), and Join On clause
* @param MatchTable: For input type 1 it will be 2d array. This 2d array will be output of DB_GetReadInput function, when called with input type 2
* @param MatchTable: For input type 2 it will be array. If not blank then resulting matchtable inputs will be added to this array
* @param CollectFields:
* @return For inputType 1: The required array which can be used as input for DB_Read
* 		  For inputType 2: The required array which will be element of matchTable array	
*/
function DB_GetReadInput($Table, $CollectFields, $clause, $inputType = 1,  $MatchTable = '')
{
	if($inputType	==	1){
		$input	=	array('Table' => $Table, 'Fields' =>	$CollectFields, 'clause'	=> $clause);
		if($MatchTable != ''){
			$input['MatchTable']	=	$MatchTable;
		}
	}	
	else
		if($MatchTable != ''){
			$MatchTable[]	=	array('Table' => $Table, 'Collect' =>	$CollectFields, 'clause'	=> $clause);
			$input	=	$MatchTable;
		}	
		else
			$input	=	array(array('Table' => $Table, 'Collect' =>	$CollectFields, 'clause'	=> $clause));
